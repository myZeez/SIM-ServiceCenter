...
class DiagnosisChat extends Component
{
    ...
    /**
     * Select a specific service to book
     */
    public function selectService(string $key): void
    {
        $cat = $this->selectedServiceCategory;
        if (!$cat || !isset(self::SERVICE_MENU[$cat]['services'][$key])) return;
        $this->selectedServiceKey = $key;
        $this->selectedServiceData = self::SERVICE_MENU[$cat]['services'][$key];
        $this->state = 'service_booking';
    }

    /**
     * Save service inquiry booking to database
     */
    ...
    {
        $deviceLabel = self::DEVICE_TYPES[$this->selectedDeviceType]['label'] ?? 'Perangkat';
